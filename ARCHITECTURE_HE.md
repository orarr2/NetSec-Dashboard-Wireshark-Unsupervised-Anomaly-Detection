...
| 3 | שופטים: Groq + Ollama בלבד | **מטריצת ספקי LLM חינמיים** (Gemini, Cerebras, OpenRouter, GitHub Models, Mistral ועוד). **המשתמש בוחר אילו שופטים ירוצו** - אחד, שניים או כולם - וכולם רצים והתוצאות חוזרות במייל. אין שום מנגנון fallback אוטומטי (סעיף 6) |
...
7. **פערים שנמצאו בקוד ורלוונטיים לתכנון:**
   ...
   - `docs/VM_DEPLOYMENT.md` מכיל כתובות IP חיות ושם קובץ מפתח
...
8. **מכסת Groq** - v1 ציין 100k tokens ליום; ‏`VM_DEPLOYMENT.md`
...
- **חשבון נפח אמיתי** (הקלטה רציפה 24/7, לפי הקצב שנמדד):
  ‏14.2GB ליום, כ-99GB לשבוע. **המכונה בפועל היא ‏boot volume של 100GB
  בלבד** (ראה סעיף 14), ואחרי OS, ‏Docker ומודל Ollama נשארים ~75GB.
  לכן:
  - **א. (הוכרע מחדש, IDX-02+03)** לא מוסיפים block volume - נשארים על
    ה-boot של 100GB כדי **לא לחצות את גבול ה-200GB החינמי ולשלם**
    (סעיף 14). זה מספיק בדיוק כי ההקלטה כרגע היא **sessions יזומים**
    (IDX-01), לא רציפה - כל session הוא עשרות עד מאות MB, לא GB.
  - **ב.** אם בעתיד רוצים הקלטה רציפה: לקצר retention ל-3-4 ימים, או
    להוסיף block volume של **עד 100GB בלבד** (100 boot + 100 block =
    200GB, עדיין חינם; 150GB כבר עולה כסף).
  - **ג.** הקלטה רציפה אמיתית שייכת ל-Pi 5 עם כרטיס SD גדול (IDX-01,
    שלב י'), לא ל-VM החינמית.
...
```
 ...
 │ buffer     │            │ PCAP - 7 ימים    │          │ (מקומי,    │   │ (דמו ציבורי) │
...
| **Ollama על ה-VM** | מקומי `:11434` | qwen2.5:14b, phi4:14b, gemma3:12b, mistral-nemo:12b, llama3.1:8b, deepseek-r1:14b | ללא מגבלה, חינם תמיד | ‏24GB RAM מריצים 14B Q4 בנוחות; איטי (~80s למועמד ב-ARM) אך בלתי-נגמר. שופט שאפשר לבחור, לא fallback |
...
### 6.3 בחירת השופטים ומעקב שימוש (אין fallback)

**עיקרון מחייב: אין שום מנגנון fallback אוטומטי.** בחירת השופטים היא
של המשתמש בלבד. מגדירים ב-`LLM_JUDGE_PANEL` אילו שופטים ירוצו - אחד,
שניים או כולם - ו**כל** שופט שנבחר רץ, ו**כל** הפסיקות חוזרות (במייל
ובדוח). המשתמש פשוט ממתין ומקבל את כל התוצאות. שופט שנכשל **מדוּוח**
בתוצאת הפאנל, ולעולם לא מוחלף בשקט במודל אחר.

- מנגנון הפאנל הקיים (`judge_candidates_panel`) כבר עושה בדיוק את זה:
  מריץ N שופטים ומחזיר את כולם + סבב עימות. שום שינוי בו.
- טבלת `llm_quota` ‏(סעיף 7) היא **אינפורמטיבית בלבד**: צריכת בקשות
  ו-tokens פר-ספק פר-יום, כדי שתדע כמה קרוב אתה לגבול החינמי של ספק -
  לא כדי לדלג עליו. תשובות ה-API כוללות שדה `usage` שהקליינטים היום
  זורקים; הוא יישמר לצורך המעקב הזה.
- ‏Ollama המקומי הוא פשוט **עוד שופט שאפשר לבחור** (חינמי ובלתי מוגבל,
  אך איטי), לא "רשת ביטחון" אוטומטית.
...
- **דרישות מינימום:** כל VM עם ‏Ubuntu 22.04+, ‏x86-64 או ‏ARM,
  ...
  ‏Oracle Always Free ‏(4 OCPU / ‏24GB RAM / ‏boot 100GB, ראה סעיף 14)
  הוא המסלול החינמי המומלץ ומקבל נספח מפורט; ‏AWS/Azure/Hetzner או כל
  ספק אחר עובדים זהה.
...
- **מה נפתח לרשת:** כלום ציבורי מלבד SSH. הכול מעל Tailscale
  ...
  ברירת המחדל). כללי ה-iptables הקיימים ב-`VM_DEPLOYMENT.md`
...
‏`usage` מהתשובות וכתיבת `llm_quota` (אינפורמטיבי). אין failover.
...
העלאה כפולה, ‏retention על עץ קבצים מדומה, פרופילי endpoint ומונה שימוש,
...
| ה' | פרופילי LLM (בחירת שופטים מרובת-ספקים) + מונה שימוש + פאנל הטרוגני **(בוצע)** | - (מקביל) |
...
| ספק LLM הגיע לגבול החינם | אין fallback אוטומטי. המשתמש בחר את השופטים; אם ספק מוצה, מריצים בלעדיו או ממתינים לאיפוס היומי. ‏Ollama המקומי זמין לבחירה תמיד (איטי אך חינם) |
...
‏`VM_DEPLOYMENT.md` נועדו למנוע בדיוק את זה, ואם זה בכל זאת קרה -
...
| 02+03 | שמירת גולמי ודיסק | **תוקן:** 7 ימים על ה-boot של 100GB בלבד, **בלי block volume**, כדי לא לחצות 200GB ולשלם (סעיף 14). מחיקה מוקדמת ב-85% תפוסה. הקלטה רציפה שדורשת יותר - שייכת ל-Pi (שלב י') |
...
## 14. המכונה בפועל, שמירה על חינם, וזרימת הנתונים

### 14.1 המפרט האמיתי של המכונה

לפי מסך ה-Oracle Cloud (netsec-agent):
...
| RAM | 24GB |
| רוחב פס רשת | 4 Gbps |
| דיסק | **Boot volume 100GB** (block storage; אין דיסק מקומי/NVMe) |
| אזור | Israel Central (Jerusalem) |
| הצפנה in-transit | Enabled |

זה בדיוק מלוא הקצאת ה-ARM החינמית של Oracle (4 OCPU + 24GB). המספרים
שהופיעו קודם ("200GB", "block volume 100-150GB") היו **גבול התוכנית**,
לא המכונה שלך - המכונה שלך היא 100GB boot. כל התכנון עודכן ל-100GB.

### 14.2 איך נשארים ב-$0 מוחלט (הכלל החשוב ביותר)

תקציב ה-Always Free שאסור לחצות, אחרת מתחילים לחייב:

| משאב | גבול חינם | המצב שלך | פעולה למניעת חיוב |
|---|---|---|---|
| ‏OCPU ‏ARM | 4 | 4 (מלא) | **אל תיצור A1 שנייה** - תחרוג |
| RAM | 24GB | 24GB (מלא) | תקציב ה-RAM בסעיף 6/D6 נשאר ~17GB |
| אחסון block כולל | **200GB** | 100GB (boot) | **אל תוסיף volume שמעלה מעל 200GB** |
| תעבורה יוצאת | 10TB/חודש | זניח | לא תרחיש מעשי |
| מכונות x86 micro | 2 (1GB) | - | אפשר להשתמש בחינם ל-watchdog |

**שתי הגנות אקטיביות שכדאי להפעיל בקונסולת Oracle:**
1. **חשבון Always Free בלבד** (לא לשדרג ל-Pay As You Go) - במצב זה
   Oracle פשוט **חוסמת** יצירת משאב בתשלום, אז אי-אפשר לטעות ולשלם.
2. אם כן שדרגת ל-PAYG: הגדר **Budget + Alert על $1** (Billing →
   Budgets) כדי לקבל מייל ברגע שמשהו מתחיל לצבור עלות.

**בקוד עצמו:** מנגנון ה-`retention` (שלב ד') מוחק גולמי מעל 7 ימים
**וגם** כשהדיסק חוצה 85% - כך שהדיסק לעולם לא מתמלא ואף כתיבה לא
"נשפכת" למשאב בתשלום. `NETSEC_MAX_UPLOAD_GB` חוסם קובץ ענק בודד.

### 14.3 איך התקשורת מהמכונה ואליה עובדת

הכול מעל **Tailscale** (רשת פרטית מוצפנת); שום פורט לא ציבורי מלבד SSH.

**קבצים נכנסים לניתוח (upload):**
```
מחשב/חיישן  ──HTTP חתום מעל Tailscale──►  VM:8766  /v1/pcap
```
- מריצים `python3 tools/upload_pcap.py capture.pcapng` (או כפתור
  הדשבורד). הקובץ מוזרם עם חתימת HMAC, מאומת ב-sha256, נכנס לתור.
- אין תקרת גודל (זה הפתרון ל-25MB). ‏scp נשאר כמסלול גיבוי.

**עיבוד:** ה-`worker` שולף מהתור, מריץ את הצינור המלא, כותב ל-DB.

**קבצים יוצאים מהמכונה (results):**
- **מייל** - הדוח המלא (כל פסיקות השופטים שבחרת) נשלח ב-SMTP. זה
  המסלול העיקרי: אתה מעלה, ממתין, ומקבל הכול במייל.
- **‏API** - `GET /v1/reports/{id}.html|pdf|json|map` ו-`/v1/sessions/{id}`
  מהדפדפן מעל Tailscale.
- **מחברת** - `load_session_from_api(session_id)` מושכת את התוצאה.
- הכול נשמר גם על דיסק ה-VM תחת `/srv/netsec/reports/`.

### 14.4 הצעות פרקטיות לניצול המכונה 24/7 (הכול בחינם)

1. **עוד מודלי LLM מקומיים ב-Ollama** - ‏24GB RAM מריצים בנוחות מודל
   14B אחד (או שניים קטנים במקביל). מומלצים ל-ARM:
   `ollama pull qwen2.5:14b` · `phi4:14b` · `gemma2:9b` ·
   `mistral-nemo:12b` · `deepseek-r1:14b` · `llama3.1:8b`. כל אחד
   הופך לשופט שאפשר לבחור ב-`LLM_JUDGE_PANEL` - חינם ובלתי מוגבל.
2. **הרצת הניתוח על המכונה** במקום על הלפטופ - זה כל הרעיון של
   Tier 1: הלפטופ יכול להיות כבוי, ה-VM ממשיכה לקלוט ולנתח.
3. **Self-hosted GitHub Actions runner** על ה-VM - ‏CI על 4 ליבות ARM
   במקום לשרוף דקות חינמיות של GitHub.
4. **אחסון וארכוב** - הדוחות, אינדקס השדות הדחוס (שלב ד') וה-DB
   נשמרים על המכונה; אפשר להוסיף Oracle Object Storage (20GB חינם)
   לארכוב ארוך-טווח מחוץ ל-boot.
5. **n8n** (כבר מותקן) - אוטומציות: התראה ב-Telegram/Slack, סיכום
   יומי במייל, טריגר על verdict זדוני.
6. **Jupyter/Dash מרוחק** מעל Tailscale - לצפות בדשבורד מהנייד בלי
   שהלפטופ דלוק.
7. **‏cron ללילה** - חישוב baseline מחדש (שלב ט'), גיזום, גיבוי DB -
   כבר מובנה ב-`retention`.
