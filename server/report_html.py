...
wrapped with a provenance header (spec section 8).
...
table at the top (file identity, capture window, versions, model), which
is what makes the stored report auditable years later.
"""
...
def render(session, markdown_body, extra=None):
    ...
    verdicts.md text; extra: optional dict appended to the header."""
    ...
    header = (
        ...
        "padding:10px 14px;margin:0 0 18px;background:#f6f8fa'>"
        "<div style='font-weight:600;margin-bottom:6px'>Session "
        "provenance</div>"
        f"<table style='font-size:12px;border-collapse:collapse'>{rows}"
        "</table></div>")
    # markdown_to_html returns a full <html> document; inject the header
    # right after <body...> so the result stays a single valid document.
    marker = body.find(">", body.find("<body")) + 1
    return body[:marker] + header + body[marker:]
