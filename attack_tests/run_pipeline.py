...
def run_security_scans(S):
    ...
    for name, cnt in (("SYN", S["syn_counter"]),
        ...
        for src, n in sorted(cnt.items(), key=lambda kv: -kv[1]):
            ...
            ratio = n / max(n_pkt, 1)
            # A horizontal spread (>20 dsts) is only a scan when the flag
            # ratio is real too: a normal client hits many hosts but
            # completes handshakes (ratio ~0.02), a scanner does not.
            # Vertical scans still fire on the ratio branch.
